...
void resetPidProfile(pidProfile_t *pidProfile)
{
    RESET_CONFIG(pidProfile_t, pidProfile,
        .profileName = { 0, },
        .pid = {
            [PID_ROLL]    = { 10, 50,  0, 50, 0, 0, 0 },
            [PID_PITCH]   = { 10, 50,  0, 50, 0, 0, 0 },
            [PID_YAW]     = { 40, 50, 10,  0, 0, 0, 0 },
        },
        .pid_mode = 1,
        .debug_axis = FD_ROLL,
        .dterm_cutoff = { 20, 20, 20 },
...
static FAST_RAM_ZERO_INIT float Ftau;
...
void pidInitProfile(const pidProfile_t *pidProfile)
{
    ...
    // Roll axis
    if (pidProfile->pid[PID_ROLL].Gain > 0) {
        pidCoefficient[PID_ROLL].Ki = ROLL_I_TERM_SCALE * pidProfile->pid[PID_ROLL].Gain;
        pidCoefficient[PID_ROLL].Kp = ROLL_P_TERM_SCALE * pidProfile->pid[PID_ROLL].Gain * pidProfile->pid[PID_ROLL].PI_balance;
        pidCoefficient[PID_ROLL].Kd = ROLL_D_TERM_SCALE * pidProfile->pid[PID_ROLL].Gain * pidProfile->pid[PID_ROLL].PI_balance * pidProfile->pid[PID_ROLL].PD_balance;
    }
    else {
    ...
    if (pidProfile->pid[PID_PITCH].Gain > 0) {
        ...
        pidCoefficient[PID_PITCH].Kp = ROLL_P_TERM_SCALE * pidProfile->pid[PID_PITCH].Gain * pidProfile->pid[PID_PITCH].PI_balance;
        pidCoefficient[PID_PITCH].Kd = ROLL_D_TERM_SCALE * pidProfile->pid[PID_PITCH].Gain * pidProfile->pid[PID_PITCH].PI_balance * pidProfile->pid[PID_PITCH].PD_balance;
    }
    else {
    ...
    if (pidProfile->pid[PID_YAW].Gain > 0) {
        ...
        pidCoefficient[PID_YAW].Kp = ROLL_P_TERM_SCALE * pidProfile->pid[PID_YAW].Gain * pidProfile->pid[PID_YAW].PI_balance;
        pidCoefficient[PID_YAW].Kd = ROLL_D_TERM_SCALE * pidProfile->pid[PID_YAW].Gain * pidProfile->pid[PID_YAW].PI_balance * pidProfile->pid[PID_YAW].PD_balance;
    }
    else {
    ...
    // Tail/yaw parameters
    tailCWStopGain = pidProfile->yaw_cw_stop_gain / 50.0f;
    tailCCWStopGain = pidProfile->yaw_ccw_stop_gain / 50.0f;
...
static FAST_CODE void pidApplyCyclicMode1(const pidProfile_t *pidProfile, uint8_t axis)
{
    ...
    float errorRate = pidSetpoint - gyroRate;
...
#ifdef USE_ITERM_RELAX
    if (itermRelax) {
        errorRate = applyItermRelax(axis, Ierr[axis], errorRate, gyroRate, pidSetpoint);
    }
#endif
...
{
    // Rate setpoint
    float pidSetpoint = getSetpointRate(FD_YAW);

    // Get filtered gyro rate
    float gyroRate = gyro.gyroADCf[FD_YAW];

    // Calculate I-error rate
    float errorRate = pidSetpoint - gyroRate;

    // Limit error bandwidth
    if (pidProfile->error_filter_hz[FD_YAW]) {
        errorRate = pt1FilterApply(&errorFilter[FD_YAW], errorRate);
    }
  ...
  //// P-term
    ...
    // Calculate P-component
    pidData[FD_YAW].P = pidCoefficient[FD_YAW].Kp * errorRate;
...
#ifdef USE_ITERM_RELAX
    if (itermRelax) {
        errorRate = applyItermRelax(FD_YAW, Ierr[FD_YAW], errorRate, gyroRate, pidSetpoint);
    }
#endif
    float itermDelta = errorRate * dT;

    // No accumulation if axis saturated
    if (pidAxisSaturated(FD_YAW)) {
        if (Ierr[FD_YAW] * itermDelta > 0) // Same sign and not zero
            itermDelta = 0;
    }

    // Calculate I-component - Ki NOT included in Ierr
    Ierr[FD_YAW] = constrainf(Ierr[FD_YAW] + itermDelta, -errorLimit[FD_YAW], errorLimit[FD_YAW]);
    pidData[FD_YAW].I = pidCoefficient[FD_YAW].Ki * Ierr[FD_YAW];

    // Apply I-term decay
#ifdef USE_ITERM_DECAY
    if (!isSpooledUp()) {
        Ierr[FD_YAW] -= Ierr[FD_YAW] * itermDecay;
    }
#endif
  ...
  //// F-term (HPF / BW-limited D)
    ...
    // Calculate F-term
    float Fterm = (pidSetpoint - Fint) * Ftau;
    Fint += Fterm;

    // Calculate feedforward component
    pidData[FD_YAW].F = pidCoefficient[FD_YAW].Kf * Fterm;


  //// PID Sum

    // Calculate PID sum
    pidData[FD_YAW].Sum = pidData[FD_YAW].P + pidData[FD_YAW].I + pidData[FD_YAW].D + pidData[FD_YAW].F;

    // Save data
    pidData[FD_YAW].Setpoint = pidSetpoint;
    pidData[FD_YAW].GyroRate = gyroRate;
}
...
static FAST_CODE void pidApplyCyclicMode2(const pidProfile_t *pidProfile, uint8_t axis)
{
  ...
  //// I-term
    ...
    float itermErrorRate = pidSetpoint - itermGyroRate;
    ...
    // Apply I-term relax
    if (itermRelax) {
        itermErrorRate = applyItermRelax(axis, Ierr[axis], itermErrorRate, itermGyroRate, pidSetpoint);
    }
#endif
...
static FAST_CODE void pidApplyYawMode2(const pidProfile_t *pidProfile)
{
    ...
    // Get filtered gyro rate
    float gyroRate = gyro.gyroDtermADCf[FD_YAW];
  ...
  //// I-term
    ...
    // Get filtered gyro rate
    float itermGyroRate = gyro.gyroADCf[FD_YAW];
    ...
    // Limit error bandwidth
    if (pidProfile->error_filter_hz[FD_YAW]) {
        itermErrorRate = pt1FilterApply(&errorFilter[FD_YAW], itermErrorRate);
    }

#ifdef USE_ITERM_RELAX
    // Apply I-term relax
    if (itermRelax) {
        itermErrorRate = applyItermRelax(FD_YAW, Ierr[FD_YAW], itermErrorRate, itermGyroRate, pidSetpoint);
    }
#endif
    float itermDelta = itermErrorRate * dT;
